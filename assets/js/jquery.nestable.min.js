...
!function (a, b, c, d) {
    ...
    var f = ("ontouchstart"in c.documentElement, function () {
    ...
    }()), g = "mousedown touchstart MSPointerDown pointerdown", h = "mousemove touchmove MSPointerMove pointermove", i = "mouseup touchend touchcancel MSPointerUp MSPointerCancel pointerup pointercancel", j = {
        listNodeName: "ol",
        itemNodeName: "li",
        rootClass: "dd",
        listClass: "dd-list",
    ...
    e.prototype = {
        init: function () {
            var b = this;
            b.reset(), b.el.data("nestable-group", this.options.group), b.placeEl = a('<div class="' + b.options.placeClass + '"/>'), a.each(this.el.find(b.options.itemNodeName), function (c, d) {
                b.setParent(a(d))
            }), b.el.on("click", "button", function (c) {
                if (!(b.dragEl || "button"in c && 0 !== c.button)) {
                    var d = a(c.currentTarget), e = d.data("action"), f = d.parent(b.options.itemNodeName);
            ...
            var e, g, h, i, j, k = this.options, l = this.mouse;
            this.dragEl.css({
            ...
            l.dirAx !== m ? (l.distAxX = 0, l.distAxY = 0) : (l.distAxX += Math.abs(l.distX), 0 !== l.dirX && l.dirX !== l.lastDirX && (l.distAxX = 0), l.distAxY += Math.abs(l.distY), 0 !== l.dirY && l.dirY !== l.lastDirY && (l.distAxY = 0)), l.dirAx = m, l.dirAx && l.distAxX >= k.threshold && (l.distAxX = 0, h = this.placeEl.prev(k.itemNodeName), l.distX > 0 && h.length && !h.hasClass(k.collapsedClass) && (e = h.find(k.listNodeName).last(), j = this.placeEl.parents(k.listNodeName).length, j + this.dragDepth <= k.maxDepth && (e.length ? (e = h.children(k.listNodeName).last(), e.append(this.placeEl)) : (e = a("<" + k.listNodeName + "/>").addClass(k.listClass), e.append(this.placeEl), h.append(e), this.setParent(h)))), l.distX < 0 && (i = this.placeEl.next(k.itemNodeName), i.length || (g = this.placeEl.parent(), this.placeEl.closest(k.itemNodeName).after(this.placeEl), g.children().length || this.unsetParent(g.parent()))));
            var n = !1;
            if (f || (this.dragEl[0].style.visibility = "hidden"), this.pointEl = a(c.elementFromPoint(d.pageX - c.body.scrollLeft, d.pageY - (b.pageYOffset || c.documentElement.scrollTop))), f || (this.dragEl[0].style.visibility = "visible"), this.pointEl.hasClass(k.handleClass) && (this.pointEl = this.pointEl.parent(k.itemNodeName)), this.pointEl.hasClass(k.emptyClass))n = !0; else if (!this.pointEl.length || !this.pointEl.hasClass(k.itemClass))return;
