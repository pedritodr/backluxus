...
class CI_Session_mongodb_driver extends CI_Session_driver implements SessionHandlerInterface
{
	...
	public function __construct(&$params)
	{
		...
		$this->_config['save_path']  = $config['sess_collection_name'];
		...
		foreach (array('sess_driver', 'sess_encrypt_cookie', 'sess_use_database', 'sess_table_name', 'sess_expiration', 'sess_expire_on_close', 'sess_match_ip', 'sess_match_useragent', 'sess_cookie_name', 'cookie_path', 'cookie_domain', 'cookie_secure', 'sess_time_to_update', 'time_reference', 'cookie_prefix', 'encryption_key') as $key) {
			$this->$key = (isset($params[$key])) ? $params[$key] : $this->CI->config->item($key);
		}
		$this->CI->load->library('mongo_db');
		$this->_db = $this->CI->mongo_db;
	...
	public function read($session_id)
	{
		...
		if ($this->_get_lock($session_id) !== FALSE) {
			...
			$conditions['id'] = (string)$session_id;
			if ($this->_config['match_ip']) {
				$conditions['ip_address'] = (string) $_SERVER['REMOTE_ADDR'];
			}

			$result = $this->_db->where($conditions)->get($this->_config['sess_collection_name']);
	...
	public function write($session_id, $session_data)
	{
		...
		if ($this->_row_exists === FALSE) {
			...
			$this->_db->insert($this->_config['sess_collection_name'], $insert_data);
			$this->_fingerprint = md5($session_data);
			return $this->_row_exists = TRUE;
		}

		$data_conditions['id'] = (string)$session_id;
		if ($this->_config['match_ip']) {
			$data_conditions['ip_address'] = $_SERVER['REMOTE_ADDR'];
		}
		...
		if ($this->_db->where($data_conditions)->set($update_data)->update($this->_config['sess_collection_name'])) {
			$this->_fingerprint = md5($session_data);
			return TRUE;
		}
	...
	public function destroy($session_id)
	{
		if ($this->_lock) {
			...
			$this->_db->delete($this->_config['sess_collection_name']);
			return (true) ? ($this->close() && $this->_cookie_destroy()) : FALSE;
		}
	...
	{
		$valor_time = (float)(time() - $maxlifetime);
		$result = $this->CI->mongo_db->where_lt('timestamp', (float)$valor_time)->delete_all($this->_config['sess_collection_name']);
		return $result;
	}
