        ...
        <div class="col-xs-12">
            <div class="statbox widget box box-shadow">
                <div class="widget-header">
                    <h6 class="text-simple"> <?= translate('edit_credit_lang') ?></h6>
                </div>
            ...
            <div class="statbox widget box box-shadow" style="margin-top: 25px;">
                <div class="widget-header">
                    <h3 class="text-simple"><?= translate('invoice_lang') ?> <span>
                            ...
                            <button class="btn btn-outline-info mb-2" onclick="handleSubmitUpdateCredit()"><?= translate('update_credit_lang') ?></button>
                        </span></h3>
                </div><!-- /.box-header -->
                <div class="widget-content widget-content-area">
...
<script>
    let uploadImages;
    let creditObject = <?= json_encode($credit) ?>;
    $(() => {
        ...
        let arrImagesEdit = [];
        creditObject.images.forEach(element => {
            arrImagesEdit.push(baseUrl + element.img);
        });

        uploadImages = new FileUploadWithPreview('uploadImages', {
            showDeleteButtonOnImages: true,
            presetFiles: arrImagesEdit
        })
        handleLoadCredit();
    ...
    const handleLoadCredit = async () => {
        invoices = [];
        $('#markings').val(creditObject.marking.marking_id);
        $('#markings').prop('disabled', true);
        result = await searchInvoice(creditObject.marking.marking_id);
        ...
        if (result.status == 200) {
            ...
            if (result.data.length > 0) {
                ...
                result.data.forEach(item => {
                    stringInvoice += '<option  itemId="' + encodeB64Uft8(JSON.stringify(item)) + '" value="' + item.invoice + '"> Fact-' + item.number_invoice + '</option>';
                });
                $('#selectInvoice').append(stringInvoice);
                $('#selectInvoice').val(creditObject.inovice.invoice);
                handleOnChangeInvoice();
                $('#selectInvoice').prop('disabled', true);
            }
        } else {
            Swal.close();
            swal({
    ...
    const handleOnChangeInvoice = () => {
        let invoice = $('select[id=selectInvoice] option').filter(':selected').attr('itemId');
        ...
        invoice.details.forEach(item => {
            stringFarms += '<option itemId="' + encodeB64Uft8(JSON.stringify(item)) + '" value="' + item.id + '">' + item.farm.name_legal + ' | ' + item.farm.name_commercial + '</option>'
        });
        $('#selectFarms').append(stringFarms);
        $('#selectFarms').val(creditObject.farm.id);
        $('#selectFarms').prop('disabled', true);
    ...
    const handleOnChangeFarm = () => {
        let farm = $('select[id=selectFarms] option').filter(':selected').attr('itemId');
                ...
                if (box.varieties.length > 0) {
                    box.varieties.forEach((element, indice, varieties) => {
                        ...
                        if (element.credit !== undefined) {
                            if (element.credit) {
                                arrItemsCredit.push(element.credit);
                                textVariety += '<div class="edit-item-invoices">';
                                textVariety += '<button class="btn btn-primary" style="display:none" id="btnAddCredit' + element.id + '" onclick=handleModalAddCredit("' + index + '","' + indice + '")><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-3"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></button>';
                                textVariety += '<button class="btn btn-warning" id="editBtnCredit' + element.id + '" onclick=handleModalEditItemCredit("' + index + '","' + indice + '")><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button>';
                                textVariety += '<button class="btn btn-danger" id="cancelBtnCredit' + element.id + '" onclick=handleCancelDeleteItem("' + index + '","' + indice + '")><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-corner-up-left"><polyline points="9 14 4 9 9 4"></polyline><path d="M20 20v-7a4 4 0 0 0-4-4H4"></path></svg></button>';
                                textVariety += '</div>';
                            } else {
                                textVariety += '<div class="edit-item-invoices">';
    ...
    let description = creditObject.description;
    ...
    const handleSubmitUpdateCredit = async () => {
        if (arrItemsCredit.length == 0) {
            const toast = swal.mixin({
                toast: true,
                position: 'top-end',
            ...
            Swal.fire({
                title: 'Completando operación',
                html: '<h5 id="textCreateCredit">Actualizando crédito...</h5>',
                imageUrl: '<?= base_url("assets/img/cargando.gif") ?>',
                imageAlt: 'No realice acciones sobre la página',
                showConfirmButton: false,
                allowOutsideClick: false,
            ...
            let items = JSON.stringify(creditObject.items);
            let data = {
                arrItemsCredit,
                description,
                items,
                creditId: creditObject.credit_id,
                invoiceId: creditObject.inovice.invoice
            };
            setTimeout(async () => {
                let response = await updateCredit(data);
                ...
                if (response.status == 200) {
                    ...
                    let responseDelete = await deleteImagen(JSON.stringify(creditObject.images));
                    Swal.fire({
                        type: 'success',
    ...
    const deleteImagen = async (images) => {
        return $.ajax({
            type: 'POST',
            url: "<?= site_url('credit/delete_images') ?>",
            data: {
                images
            },
            success: function(result) {
