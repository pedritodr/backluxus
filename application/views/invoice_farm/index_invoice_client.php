...
<script>
                    ...
                    if (box.varieties.length > 0) {
                        box.varieties.forEach(element => {
                            ...
                            if (element.priceClient !== undefined) {
                                ...
                                if (status !== '2') {
                                    const validRol = [1, 2, 5, 4, 3].includes(Number('<?= $this->session->userdata('role_id') ?>'));
                                    if (validRol) {
    ...
    const handleSendClient = (id) => {
        swal({
            ...
            type: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Enviar invoice al cliente',
            padding: '2em'
        }).then(function(result) {
            if (result.value) {
                let urlSend = '<?= site_url("invoice_farm/send_invoice_client/"); ?>' + id;
