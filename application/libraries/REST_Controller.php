...
abstract class REST_Controller extends CI_Controller
{
    ...
    protected function _detect_output_format()
    {
        ...
        $matches = array();
        $end_first = end($this->_get_args);
        $a_keys = array_keys($this->_get_args);
        $end_second = end($a_keys);

        if (!$this->_get_args and preg_match($pattern, $this->uri->uri_string(), $matches)) {
            return $matches[1];
        } // Check if a file extension is used
        elseif ($this->_get_args and !is_array($end_first) and preg_match($pattern, $end_second, $matches)) {
